...
class ResourceManager:
    ...
    async def _detect_gpu_resources(self) -> None:
        ...
        await self._detect_rocm_resources()
        await self._detect_opencl_devices()

    async def _detect_rocm_resources(self) -> None:
        """Detect GPUs exposed via ROCm tooling."""
        if not shutil.which("rocm-smi"):
            logger.debug("rocm-smi not available; skipping ROCm detection")
            return
        try:
            result = await self._run_command("rocm-smi --showproductname")
        except (FileNotFoundError, subprocess.SubprocessError) as exc:
            logger.debug("ROCm detection failed: %s", exc)
            return
        for line in result.splitlines():
            clean = line.strip()
            if not clean or "GPU" not in clean or ":" not in clean:
                continue
            ident, name = clean.split(":", 1)
            name = name.strip()
            index = "0"
            if "[" in ident and "]" in ident:
                index = ident.split("[")[-1].split("]")[0]
            gpu_id = f"rocm:{index}"
            resource = Resource(
                type=ResourceType.ROCM,
                id=gpu_id,
                name=f"{name} (ROCm)",
                capacity=1.0,
                available=1.0,
            ...
            self.resources[(ResourceType.ROCM, gpu_id)] = resource
            logger.info("Detected ROCm GPU: %s (%s)", name, gpu_id)

    async def _detect_opencl_devices(self) -> None:
        """Detect OpenCL devices exposed via clinfo."""
        if not shutil.which("clinfo"):
            logger.debug("clinfo not available; skipping OpenCL detection")
            return
        try:
            result = await self._run_command("clinfo -l")
        except (FileNotFoundError, subprocess.SubprocessError) as exc:
            logger.debug("OpenCL detection failed: %s", exc)
            return
        for line in result.splitlines():
            clean = line.strip()
            if not clean.lower().startswith("device #") or ":" not in clean:
                continue
            ident, name = clean.split(":", 1)
            name = name.strip()
            try:
                index = ident.split("#", 1)[1].split()[0]
            except (IndexError, ValueError):
                index = "0"
            device_id = f"opencl:{index}"
            resource = Resource(
                type=ResourceType.OPENCL,
                id=device_id,
                name=f"{name} (OpenCL)",
                capacity=1.0,
                available=1.0,
            ...
            self.resources[(ResourceType.OPENCL, device_id)] = resource
            logger.info("Detected OpenCL device: %s (%s)", name, device_id)
